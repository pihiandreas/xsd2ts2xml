...
<xsl:stylesheet version="2.0"
	...
	<xsl:import href="F10_Transaction.xsl" />
	...
	<xsl:template name="MessageTemplate"
		match="soapenv:Envelope/soapenv:Body/urn:SendMessageRequest/urn:MessageContainer/urn:Payload/urn1:RequestSettlementDataInfoMessage">
		<entity>
			<_name>Message</_name>
			<_type>MESSAGE</_type>
			<xsl:if test="urn1:RequestSettlementDataInfoMessage/urn1:Header/urn2:DocumentType">
				<ExternalMessageType>
					<xsl:value-of select="urn1:RequestSettlementDataInfoMessage/urn1:Header/urn2:DocumentType"/>
				</ExternalMessageType>
			</xsl:if>
			<xsl:if test="urn1:RequestSettlementDataInfoMessage/urn1:Header/urn2:Identification">
				<ExternalMessageID>
					<xsl:value-of select="urn1:RequestSettlementDataInfoMessage/urn1:Header/urn2:Identification"/>
				</ExternalMessageID>
			</xsl:if>
			<xsl:if test="urn1:RequestSettlementDataInfoMessage/urn1:Header/urn2:SenderRoutingInformation">
				<ExternalMessageReference>
					<xsl:value-of select="urn1:RequestSettlementDataInfoMessage/urn1:Header/urn2:SenderRoutingInformation"/>
				</ExternalMessageReference>
			</xsl:if>
			<xsl:variable name="ProcessRole" select="urn1:RequestSettlementDataInfoMessage/urn1:ProcessEnergyContext/urn3:EnergyBusinessProcessRole"/>
			...
			<xsl:if test="urn1:RequestSettlementDataInfoMessage/urn1:ProcessEnergyContext/urn3:EnergyIndustryClassification">
				<xsl:choose>
					<xsl:when test="urn1:RequestSettlementDataInfoMessage/urn1:ProcessEnergyContext/urn3:EnergyIndustryClassification='23'">
						<Industry>1</Industry>
					</xsl:when>
					<xsl:otherwise>
						<Industry>
							<xsl:value-of select="urn1:RequestSettlementDataInfoMessage/urn1:ProcessEnergyContext/urn3:EnergyIndustryClassification"/>
						</Industry>			
					</xsl:otherwise>
				</xsl:choose>
			</xsl:if>
			<xsl:if test="urn1:RequestSettlementDataInfoMessage/urn1:ProcessEnergyContext/urn3:EnergyBusinessProcess">
				<xsl:choose>
					<xsl:when test="urn1:RequestSettlementDataInfoMessage/urn1:ProcessEnergyContext/urn3:EnergyBusinessProcess='DH-521'">
						<BusinessProcess>32</BusinessProcess>
					</xsl:when>
					<xsl:when test="urn1:RequestSettlementDataInfoMessage/urn1:ProcessEnergyContext/urn3:EnergyBusinessProcess='DH-522'">
						<BusinessProcess>33</BusinessProcess>
					</xsl:when>
					<xsl:otherwise>
						<BusinessProcess>
							<xsl:value-of select="urn1:RequestSettlementDataInfoMessage/urn1:ProcessEnergyContext/urn3:EnergyBusinessProcess"/>
						</BusinessProcess>			
					</xsl:otherwise>
				</xsl:choose>
			</xsl:if>
			<_entities>
				<_entities_element>
					...
					<xsl:variable name="PhysicalSenderID" select="enrich:getPhysicalSenderID(urn1:RequestSettlementDataInfoMessage/urn1:Header/urn2:PhysicalSenderEnergyParty/urn2:Identification, $UserName,$TenantCode, $ProcessRole)"/>
					<PhysicalSenderID>
						<xsl:value-of select="$PhysicalSenderID" />
					</PhysicalSenderID>
					<xsl:variable name="JuridicalSender" select="urn1:RequestSettlementDataInfoMessage/urn1:Header/urn2:JuridicalSenderEnergyParty/urn2:Identification"/>
					<JuridicalSenderID>
						<xsl:choose>
