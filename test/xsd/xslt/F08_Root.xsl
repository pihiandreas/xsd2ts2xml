...
<xsl:stylesheet version="2.0"
	...
	xmlns:urn1="urn:fi:Datahub:mif:metering:F08_RequestMeasuredDataInfo:v1" 
	...
	xmlns:urn4="urn:fi:Datahub:mif:metering:F08_RequestMeasuredDataInfo:elements:v1"
	xmlns:uuid="java:java.util.UUID"
	xmlns:enrich="java:com.cgi.cms.mhb.utl.mtc.enrich.XMLEnricher"
	exclude-result-prefixes="enrich uuid">

	<xsl:import href="F08_Transaction.xsl" />
	...
	<xsl:template name="MessageTemplate"
		match="soapenv:Envelope/soapenv:Body/urn:SendMessageRequest/urn:MessageContainer/urn:Payload/urn1:RequestMeasuredDataInfoMessage">
		<entity>
			<_name>Message</_name>
			<_type>MESSAGE</_type>
			<xsl:if test="urn1:RequestMeasuredDataInfo/urn1:Header/urn2:DocumentType">
				<ExternalMessageType>
					<xsl:value-of select="urn1:RequestMeasuredDataInfo/urn1:Header/urn2:DocumentType"/>
				</ExternalMessageType>
			</xsl:if>
			<xsl:if test="urn1:RequestMeasuredDataInfo/urn1:Header/urn2:Identification">
				<ExternalMessageID>
					<xsl:value-of select="urn1:RequestMeasuredDataInfo/urn1:Header/urn2:Identification"/>
				</ExternalMessageID>
			</xsl:if>
			<xsl:if test="urn1:RequestMeasuredDataInfo/urn1:Header/urn2:SenderRoutingInformation">
				<ExternalMessageReference>
					<xsl:value-of select="urn1:RequestMeasuredDataInfo/urn1:Header/urn2:SenderRoutingInformation"/>
				</ExternalMessageReference>
			</xsl:if>
			...
			<xsl:variable name="ProcessRole" select="urn1:RequestMeasuredDataInfo/urn1:ProcessEnergyContext/urn3:EnergyBusinessProcessRole"/>
...
<!--				<xsl:choose>
			...
			<xsl:if test="urn1:RequestMeasuredDataInfo/urn1:ProcessEnergyContext/urn3:EnergyIndustryClassification">
<!--				<xsl:choose>
					<xsl:when test="urn1:RequestMeasuredDataInfo/urn1:ProcessEnergyContext/urn3:EnergyIndustryClassification='23'">
						<Industry>1</Industry>
					</xsl:when>
					<xsl:otherwise>
						<Industry>
							<xsl:value-of select="urn1:RequestMeasuredDataInfo/urn1:ProcessEnergyContext/urn3:EnergyIndustryClassification"/>
						</Industry>			
					</xsl:otherwise>
				</xsl:choose>-->
				<Industry>
			...
			<xsl:if test="urn1:RequestMeasuredDataInfo/urn1:ProcessEnergyContext/urn3:EnergyBusinessProcess">
<!--				<xsl:choose>
					<xsl:when test="urn1:RequestMeasuredDataInfo/urn1:ProcessEnergyContext/urn3:EnergyBusinessProcess='DH-221'">
						<BusinessProcess>12</BusinessProcess>
					</xsl:when>
					<xsl:when test="urn1:RequestMeasuredDataInfo/urn1:ProcessEnergyContext/urn3:EnergyBusinessProcess='DH-222'">
						<BusinessProcess>13</BusinessProcess>
					</xsl:when>
					<xsl:when test="urn1:RequestMeasuredDataInfo/urn1:ProcessEnergyContext/urn3:EnergyBusinessProcess='DH-223'">
						<BusinessProcess>14</BusinessProcess>
					</xsl:when>
					<xsl:when test="urn1:RequestMeasuredDataInfo/urn1:ProcessEnergyContext/urn3:EnergyBusinessProcess='DH-523'">
						<BusinessProcess>34</BusinessProcess>
					</xsl:when>
					<xsl:otherwise>
						<BusinessProcess>
							<xsl:value-of select="urn1:RequestMeasuredDataInfo/urn1:ProcessEnergyContext/urn3:EnergyBusinessProcess"/>
						</BusinessProcess>			
					</xsl:otherwise>
				</xsl:choose>-->
				<BusinessProcess>
			...
			<_entities>
				<_entities_element>
					...
					<xsl:variable name="PhysicalSenderID" select="enrich:getPhysicalSenderID(urn1:RequestMeasuredDataInfo/urn1:Header/urn2:PhysicalSenderEnergyParty/urn2:Identification, $UserName,$TenantCode, $ProcessRole)"/>
					<PhysicalSenderID>
						<xsl:value-of select="$PhysicalSenderID" />
					</PhysicalSenderID>
					<xsl:variable name="JuridicalSender" select="urn1:RequestMeasuredDataInfo/urn1:Header/urn2:JuridicalSenderEnergyParty/urn2:Identification"/>
					<JuridicalSenderID>
						<xsl:choose>
